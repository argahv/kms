...
export function ParentDashboard({ parent }: ParentDashboardProps) {
  ...
  const childAttendance = attendanceRecords.filter((record) =>
    record.id.startsWith(selectedChild.id)
  );
  const childMarks = marks.filter((mark) =>
    mark.id.startsWith(selectedChild.id)
  );
    ...
    <div className='teacher-dashboard min-h-screen p-6 animate-fadeIn'>
      ...
      <div className='mb-4'>
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedChild(
              children.find((child) => child.id === e.target.value) || null
            )
          }
